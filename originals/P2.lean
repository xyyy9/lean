  ...
  · use Set.eq_singleton_iff_unique_mem.2 ⟨?_,λb g=>by_contra$ g.2.2.rec λY S i=>S.rec λL D=>?_⟩
    ...
    · suffices:b.1^φ (b.1*b.2+1)%(b.1*b.2+1)=1%(b.1*b.2+1) ∧b.2^ φ (b.1* b.snd+1)%((b).1 * ↑(b.snd)+1)= 1% (b.1*b.snd + 1)
      · /- Then ab + 1 | a ^ (Nφ(ab + 1)) + b and
          ab + 1 | b ^ (Nφ(ab + 1)) + a. -/
        absurd D.2.2 (φ (b.1*b.2+1)*L) (by nlinarith [(b.fst *b.2+1).totient_pos ↑ Fin.size_pos'])
    ...
    · simp_all
      ...
      · use H$ D.2.2 (φ _*(L+1)-1) (L.le_sub_of_add_le (by nlinarith[(b.1* b.2+1).totient_pos Nat.succ_pos']))▸(((Nat.dvd_gcd) ( this).1)) this.right
      cases B:Nat.exists_eq_add_of_lt$ (b.1*b.2+1).totient_pos (by continuity)
      norm_num[*, mul_add,Nat.pow_mod,(1).mod_eq_of_lt,pow_add,Nat.add_mod,pow_mul,Nat.dvd_iff_mod_eq_zero,Nat.mul_mod] at this⊢
      simp_all
